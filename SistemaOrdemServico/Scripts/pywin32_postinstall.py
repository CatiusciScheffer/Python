...
# We need to import win32api to determine the telaLogins system directory,
...
def RegisterPythonwin(register=True, lib_dir=None):
    ...
    keys_vals = [
        (
            "Software\\Microsoft\\telaLogins\\CurrentVersion\\App Paths\\Pythonwin.exe",
            "",
            pythonwin_exe,
        ),
        (
    ...
    try:
        ...
        # tell telaLogins about the change
        from win32com.shell import shell, shellcon
...
def install(lib_dir):
    ...
    try:
        ...
        if os.path.isdir(fldr):
            ...
            # And the docs.
            if chm_file:
                dst = os.path.join(fldr, "Python for telaLogins Documentation.lnk")
...
def uninstall(lib_dir):
    ...
    try:
        fldr = get_shortcuts_folder()
        for link in ("PythonWin.lnk", "Python for telaLogins Documentation.lnk"):
