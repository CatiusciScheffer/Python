...
class ManipularCriacaodeRelatorios():
    ...
    #     # Obter os dados da tabela
    #     cursor.execute("SELECT os_dtServico, os_codCliente, os_cliente, os_codServico, os_descServico, SUM(os_qtd) as total_quantidade, os_vlrUnit, SUM(os_total) as total_valor_total FROM tb_ordens_servicos WHERE os_faturado = 'NÃO' GROUP BY os_cliente, os_codServico, os_descServico ORDER BY os_cliente, os_codServico")
    ...
    #                             leftMargin=1.5*units.cm, rightMargin=1.5*units.cm)
    #     elements = []
    ...
    #     # Criar a tabela com os dados do banco de dados
    #     table_data = [['Data', 'Cód_Cliente', 'Cliente', 'Cód_Serv', 'Descrição do Serviço', 'Quantidade', 'Valor Unit.', 'Total']]
    #     table_data.extend(data)
        
    #     # Calcula a largura das colunas com base no conteúdo
    #     column_widths = self.calculate_column_widths(table_data)
    ...
    def gerarRelatorioOdensServicoNAOfaturadas(self, output_file):
        ...
        total_style = TableStyle([
                                ...
                                ('LINEBELOW', (0, -1), (-1, -1), 2, colors.black),  # Adicione uma linha abaixo da última linha
